...
net = SlayerLoihiMLP(params, input_size, args.hidden_size, output_size, quantize=False).to(device)
# net.load_state_dict(torch.load(args.saved_weights))
sd = OrderedDict({"fc1.weight": torch.from_numpy(np.load(args.saved_weights + "quantized-fc1.npy")),
                  "fc2.weight": torch.from_numpy(np.load(args.saved_weights + "quantized-fc2.npy"))})
